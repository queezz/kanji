...
def get_files_with_extension(
    ...
    if not recursive:
        return [path for path in Path(working_dir).iterdir() if regex.match(path.name)]
    else:
        matches = []
...
def cli() -> None:
    ...
    Returns:
        None
    """
    parser = argparse.ArgumentParser("latexclean", description=__file__.__doc__)
    parser.add_argument(
        "-r",
        "--recursive",
    ...
    files = get_files_with_extension(
        Path("."), latex_aux_extensions, recursive=args.recursive
    )
    if not args.force:
        ask_remove(files)
    else:
